...
#import "WiiSystemLanguageViewController.h"

#import "Core/Config/SYSCONFSettings.h"

@interface WiiSystemLanguageViewController ()

@end

@implementation WiiSystemLanguageViewController {
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  _lastSelected = Config::Get(Config::SYSCONF_LANGUAGE);
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath {
  if (_lastSelected != indexPath.row) {
    Config::SetBase(Config::SYSCONF_LANGUAGE, (int)indexPath.row);
